		...
		case 72://UP上方向键
			if (key > 1)//当此时选中项为第一项时，UP上方向键无效
			{
				...
				case 2:
					...
					SetCursorPosition(27, 24);//将已选中项取消之前选中项的背景色
					SetColor(3);
					std::cout << "普通模式";
...
int Controller::PlayGame()//游戏二级循环
{
	...
	/*游戏循环*/
	while (csnake->NoOverEdge() && csnake->NoHitItself()) //判断是否撞墙或撞到自身，即是否还有生命
	{
		/*
		//调出选择菜单
...
void Controller::RewriteScore()//重绘分数
{
	...
	for (int i = 0; i < (6 - bit); ++i)//将剩余位数用空格补全
	{
		std::cout << " ";
	}
	std::cout << score;
